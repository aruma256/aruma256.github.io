...
型定義では `initialize` が引数なしで定義されていたため、`ast_node` や `extensions` を渡すとエラーになっていました。
...
これが初PRでした。gem_rbs_collectionは[レビュー無しでもマージできる運用](https://www.timedia.co.jp/tech/20240513-tech/)になっていますが、初めてで心配だったので念のためレビューを依頼し、approveをもらってからマージしました。
...
型定義では `initialize` が `String` のみを受け取るように定義されていたため、`level` オプションを渡すとエラーになっていました。
...
`null` メソッドの型定義が存在しなかったため、呼び出すとエラーになっていました。
