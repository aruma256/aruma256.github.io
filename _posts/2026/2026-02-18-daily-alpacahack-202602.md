...
2/1～2/15 の Daily AlpacaHack を解いたので、解法メモを残しておきます。  
どれも「自力で解ける」～「AIに相談しつつ解ける」程度で、CTF初心者にちょうどいい、楽しめる難易度でした。  
（前半と言いつつ、勢い余って15日分になりました。）
...
結果として `alpaca+llama > 0x40` が満たされるように 4294967295, 2 などを選び、あとは書き込みまくることでシェルを起動し、フラグを入手できました。
...
となっており、単にコピペして渡すだけは改行が問題となるようです。  
